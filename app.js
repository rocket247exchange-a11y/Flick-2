// app.js
// Helper functions to normalize URLs (Dropbox -> raw=1, detect YouTube, handle Imgur albums)
function normalizeVideoUrl(url) {
  ...
  try {
    ...
    if (host.includes('dropbox.com')) {
      // force raw=1 so browser tries direct file
...
function normalizePosterUrl(url) {
  ...
  try {
    const u = new URL(url);
    const host = u.hostname.toLowerCase();
    // direct i.imgur.com or storage.googleapis.com are safe
    if (host === 'i.imgur.com' || host.includes('storage.googleapis.com')) return url;
    // if imgur album page (contains /a/), we cannot derive direct image reliably — return null so fallback UI is used
    if (host.includes('imgur.com') && u.pathname.startsWith('/a/')) return null;
    return url;
  } catch (e) {
    return url;
  }
...
// === MOVIES (Now Showing) — using storage.googleapis GIFs and i.imgur gifs where provided ===
const MOVIES = [
  {
    id: "madam-cash",
    title: "MADAM CASH",
  ...
  },
  {
    ...
    plot: "In a wasteland alternate Abuja, a demon terrorises the people and causes mass hysteria. A gripping visual & sound experience.",
    year: 2025,
...
// Coming soon items (these will be shown as GIF backgrounds with overlay)
const COMING = [
  { id: "origin-abiyoyo", title: "ORIGIN OF ABIYOYO", plot: "The origin story...", poster: "https://i.imgur.com/9Uklygc.jpeg" },
...
// Create Now Showing movie cards (standard framed cards)
function createMovieCard(m) {
  const card = document.createElement('article');
  card.className = 'card';
  const posterUrl = normalizePosterUrl(m.poster) || 'data:image/svg+xml;charset=utf-8,<svg xmlns="http://www.w3.org/2000/svg" width="1280" height="720"><rect width="100%" height="100%" fill="%23000"/><text x="50%" y="50%" fill="%23fff" font-size="28" font-family="Arial" text-anchor="middle">Poster unavailable</text></svg>';
  card.innerHTML = `
    <div class="card-logo"><img src="https://i.imgur.com/e40IkRK.jpeg" alt="FLICK logo"></div>
    <div class="poster-wrap">
      <div class="poster-flip flipping"><img class="poster-img" src="${posterUrl}" alt="${m.title}"></div>
    </div>
    <div class="title-row">
      <div>
        <h3>${m.title}</h3>
  ...
  card.querySelector('.poster-img').addEventListener('error', (ev) => { ev.target.src = 'data:image/svg+xml;charset=utf-8,<svg xmlns="http://www.w3.org/2000/svg" width="1280" height="720"><rect width="100%" height="100%" fill="%23000"/><text x="50%" y="50%" fill="%23fff" font-size="28" font-family="Arial" text-anchor="middle">Poster unavailable</text></svg>'; });
  return card;
}

// Build grid
const gridWrap = document.getElementById('grid-wrap');
MOVIES.forEach(m => gridWrap.appendChild(createMovieCard(m)));

// poster flip staggering
document.querySelectorAll('.poster-flip').forEach((el, i) => { el.style.animationDelay = (i * 260) + 'ms'; });

// Coming Soon: GIFs as background with overlay
const comingGrid = document.getElementById('coming-grid');
COMING.forEach(c => {
  const div = document.createElement('div');
  div.className = 'coming-card';
  // use poster URL directly (gif/jpg)
  ...
  div.innerHTML = `
    <div class="coming-overlay">
      ...
      <div class="coming-text">
        <h4>${c.title}</h4>
        <p>${c.plot || 'Coming soon — subscribe to be notified.'}</p>
      </div>
      <div class="coming-actions">
        ...
        <a class="btn flickit" href="#" data-coming="${c.id}">Flick it</a>
      </div>
    </div>
  `;
  comingGrid.appendChild(div);

  // clicking Flick it on coming item opens YouTube channel (or could link to trailer)
  div.querySelector('.btn.flickit').addEventListener('click', (e)=> {
    e.preventDefault();
    window.open('https://youtube.com/@kezithelastcreator?si=AIUm9DMrV8DnBxw-','_blank');
  });
});

// Small auto-scroll for desktop
